...
def needs_fit(src_doc, media_w, media_h):
    """So' mexe no que nao cabe. Ampliar um recorte que ja' cabia foi o que
    fez a etiqueta de 97x129mm sair cortada: a area imprimivel e' menor que
    os 100x150mm nominais do preset."""
    return any(p.rect.width > media_w or p.rect.height > media_h
               for p in src_doc)
...
def main():
    ...
    if not needs_fit(src, media_w, media_h):
        if not args.quiet:
            print("  cabe na etiqueta, mandando sem alterar")
        src.close()
        return 3
